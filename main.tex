...
Expressions for these two components, produced by electrons with pitch angle $\sin\alpha$, are provided below in Eqs.~\ref{Jperp} and \ref{Jpara},
...
\noindent where
\begin{align}
\tau^{-1} &= \frac{\sqrt{3} \alpha}{4\pi}\frac{B\sin\alpha}{B_{\mathrm{crit}}}\frac{m_{e}c^{2}}{\hbar},
 & 
E_{\gamma}^{\mathrm{peak}} &= \frac{3}{2}\Gamma_{e}^2 \frac{B\sin\alpha}{B_{\mathrm{crit}}} m_{e} c^2,
\nonumber
\end{align}
and
\begin{align}
...
\frac{{\rm d}E_e^{\rm tot}}{{\rm d}t} = \frac{2\alpha}{3} n_{e}\left(\frac{B\sin\alpha}{B_{\rm crit}}\right)^{2}\frac{E_{e}^{2}}{\hbar}%\left(\frac{B_{\perp}}{B_{\rm crit}\right)^{2}%\frac{E_{e}^{2}}{\hbar},
\label{Itot_l}
\end{equation}
where the result $\int_0^{\infty} F(x) \rm{d}x = 8\pi/(9\sqrt{3})$ \citep{Longair} has been used. The above expressions can be used to compute the pitch angle averaged synchrotron cooling time for electrons, given by $\tau_c = \frac{E_e}{{\rm d}E_e/{\rm d}t}$ \citep{Taylor_Matthews}.
