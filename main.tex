...
\begin{equation} \label{eq_I_tot}
    I_{\rm tot} = \int_0^L (J_{\perp}^{\rm l} + J_{\parallel}^{\rm l}) \rm{d}l
\end{equation}
$I_{\rm tot}$ is computed by summing the contributions of $J_{\perp}^l$ and and $J_{\parallel}^l$ for each point along the line of sight. 
...
\hspace*{+9cm}                                      \includegraphics[width=0.40\linewidth]{Images/Colorbar.png}
\vspace*{0.001mm}
\caption{Log brightness of the backtracked positions for cosmic rays shown on \textbf{left} and the arrival directions of the cosmic rays from two known sources Cen A and NGC 253 on \textbf{right} (From table~\ref{Para_table} \textbf{Top} - Best Fit Parameters, {\textbf{Middle} - Lower extreme parameters)} \& {\textbf{Bottom} - Upper extreme parameters, we use $R_{\rm Mag} = $ 20~kpc since that is the maximum extent of the magnetic fields in our simulation.} \Andrew{BIG LABELS STATING "MAXIMUM" AND "MINIMUM" NEEDS ADDING TO EACH ROW.}}
\label{fig:AD_Plots}
\end{figure*}
%\newpage
% \begin{frame}
...
\caption{Diagram depicting visually the resultant ellipse, obtained from the summation of two ellipses of equal magnitude, but different orientations. The resultant ellipse dictates the resultant total and polarised intensities.}
\end{figure*}
